...
<div id="mobileSidebar" class="mobile-sidebar d-md-none">
  ...
  <div class="mobile-sidebar-footer mt-4">
    <ul class="list-unstyled">
      <li><a href="login.html" style="text-decoration: none; color: inherit;">Login</a></li>
        ...
        <?php if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])): ?>
            ...
            <div class="checkout-product-item d-flex align-items-center mb-3">
                ...
                <div class="checkout-product-details flex-grow-1">
                    <p class="checkout-product-name"><?php echo $item['name']; ?></p>
                    <p class="checkout-product-size"><?php echo $item['size']; ?></p>
                </div>
                <div class="quantity-box d-flex align-items-center border">
                                <button class="btn btn-sm quantity-decrease" data-index="<?php echo $index; ?>" data-unit-price="<?php echo $item['price']; ?>">-</button>
                                <input type="number" class="form-control form-control-sm text-center quantity-input" value="<?php echo $item['quantity']; ?>" min="1" data-index="<?php echo $index; ?>" data-unit-price="<?php echo $item['price']; ?>">
                ...
                <div class="checkout-product-price ms-3" id="price-<?php echo $index; ?>">
            ...
            <div class="checkout-subtotal-section mt-4 d-flex justify-content-between">
                <div>Total Price • <?php echo count($_SESSION['cart']); ?> items</div>
                <div id="subtotal">Rs <?php echo number_format($totalamount, 2); ?></div>
            </div>
        <?php else: ?>
            <p>Your cart is empty.</p>
        <?php endif; ?>
        <!-- Next Button -->
        <div class="checkout-next-btn mt-4">
            <a href="delivery-detail.php">
                <button class="btn btn-dark">
                    Next <span>&rarr;</span>
                </button>
            </a>
        </div>
        <div class="checkout-discount-section mt-4 mx-0">
            <div class="input-group">
                <input type="text" class="checkout-discount-input form-control" placeholder="Enter discount code">
                <button class="checkout-apply-btn btn btn-dark">Apply</button>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('.delete-item').click(function() {
        var index = $(this).data('index');
        $.ajax({
            url: '', // current page, handle it via PHP
            type: 'POST',
            data: { 
                delete_item: true, 
                index: index // Pass the index of the item to delete
            },
            success: function(response) {
