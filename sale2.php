...
if (isset($_GET["query"]) && !empty($_GET["query"])) {
    ...
    $sql = "SELECT * FROM sale_products WHERE saleName LIKE '%$search_query%'";
} else {
...
switch ($sort_order) {
    case 'alpha-asc':
        ...
        break;
    case 'alpha-desc':
        $sql .= " ORDER BY saleName DESC";
        break;
    case 'price-asc':
        $sql .= " ORDER BY CAST(REPLACE(REPLACE(salePrice, 'Rs ', ''), ',', '') AS DECIMAL) ASC";
        break;
    case 'price-desc':
        $sql .= " ORDER BY CAST(REPLACE(REPLACE(salePrice, 'Rs ', ''), ',', '') AS DECIMAL) DESC";
        break;
    case 'date-asc':
        $check_column = $conn->query("SHOW COLUMNS FROM sale_products LIKE 'createdAt'");
...
$totalProductsResult = $conn->query("SELECT COUNT(*) AS total FROM sale_products");
   ...
   <div class="container-fluid">
     ...
     <div class="collapse navbar-collapse" id="navbarNav">
       <ul class="navbar-nav ms-auto">
         <li class="nav-item">
           <a class="nav-link" href="sale2.html">Buy on Sale!</a>
         </li>
         ...
         <li class="nav-item dropdown">
           <a class="nav-link" href="HWA.html" id="whoWeAre" role="button">Who We Are<i
               class="down-arrow bi bi-caret-down-fill"></i></a>
           <ul class="dropdown-menu" aria-labelledby="whoWeAre">
             <li><a class="dropdown-item" href="faqs.html">FAQ + How we pay-it-forward ❤️</a></li>
           </ul>
         </li>
       </ul>
       <ul class="icon-list">
   ...
   <div id="cartSidebar" class="sidebar">
     <div class="sidebar-content">
       ...
       <hr>
         ...
         <p>Taxes and <a href="Privacy Policy.html" class="shipping" style="text-decoration: underline;">shipping</a> calculated at checkout</p>
...
<body>
  <div class="container text-center mt-5">
    ...
    <div class="d-flex justify-content-center align-items-center mt-3">
      <label for="sort" id="sort" class="me-2">Sort by:</label>
      <form method="GET" action="">
        <select id="sortSelect" name="sort" class="form-select w-auto" style="border-radius: 0;" onchange="this.form.submit()">
            <option value="default" <?= $sort_order == 'default' ? 'selected' : ''; ?>>Best selling</option>
            <option value="featured" <?= $sort_order == 'featured' ? 'selected' : ''; ?>>Featured</option>
            <option value="alpha-asc" <?= $sort_order == 'alpha-asc' ? 'selected' : ''; ?>>Alphabetically, A-Z</option>
            <option value="alpha-desc" <?= $sort_order == 'alpha-desc' ? 'selected' : ''; ?>>Alphabetically, Z-A</option>
            <option value="price-asc" <?= $sort_order == 'price-asc' ? 'selected' : ''; ?>>Price, low to high</option>
            <option value="price-desc" <?= $sort_order == 'price-desc' ? 'selected' : ''; ?>>Price, high to low</option>
            <option value="date-asc" <?= $sort_order == 'date-asc' ? 'selected' : ''; ?>>Date, old to new</option>
            <option value="date-desc" <?= $sort_order == 'date-desc' ? 'selected' : ''; ?>>Date, new to old</option>
        </select>
    </form>
    </div>
    <div class="d-flex justify-content-center align-items-center mt-3">
    ...
    if ($results->num_rows > 0) {
        while ($row = $results->fetch_assoc()) {
            ...
            echo '          <img src="' . $row['saleImage'] . '" class="card-img-top" alt="Item Image">';
            echo '      </a>';
            echo '      <div class="card-body">';
            echo '          <p class="card-text"><span>' . $row["saleName"] . '</span> — <span>' . $row["salePrice"] . '</span></p>';
            echo '      </div>';
            echo '  </div>';
            echo '</div>';
        }
    ...
    <ul class="pagination justify-content-center">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <li class="page-item <?= $currentPage == $i ? 'active' : ''; ?>">
                <a class="page-link" href="?page=<?= $i ?>&sort=<?= $sort_order ?>&query=<?= $search_query ?>"><?= $i ?></a>
            </li>
        <?php endfor; ?>
    </ul>
</nav>
        ...
        <div class="modal-body">
          <div class="search-bar-container">
            <div class="search-bar">
              <form action="sale2.php" method="GET" class="search-box">
